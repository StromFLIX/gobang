...
test('ranked matchmaking waits, cancels, and pairs two accounts once', async ({ browser }) => {
  ...
  await first.reload()
  await expect(first.getByText('Mina captures Felix’s pair')).toBeVisible({ timeout: 18_000 })
  ...
  const winner = first.locator('.match-replay__winner')
  await expect(winner).toContainText('Mina')
  await expect(winner.locator('.match-replay__crown')).toBeVisible()
  await expect(first.locator('.match-replay__winning-line')).toBeVisible()
  const boardBox = await first.locator('.match-replay__board').boundingBox()
  const winnerBox = await winner.boundingBox()
  expect(boardBox).not.toBeNull()
  expect(winnerBox).not.toBeNull()
  expect(winnerBox!.y + winnerBox!.height).toBeLessThan(boardBox!.y + boardBox!.height / 3)
  ...
  await expect(first.getByText('Rewinding the match')).toBeVisible({ timeout: 6_000 })
  await expect(first.getByText('Replay restarting')).toBeVisible({ timeout: 3_000 })
...
test('registered players can challenge from the leaderboard and accept in realtime', async ({
  ...
  await challenger.getByRole('button', { name: 'Explore leaderboard' }).click()
  ...
  await expect(challenger.getByRole('button', { name: `Challenge ${recipientName}` })).toBeHidden()
  ...
  await recipient.getByRole('button', { name: `Accept ${challengerName}'s challenge` }).click()
...
test('two private players receive realtime turn updates', async ({ browser }) => {
  ...
  const rulesPanel = host.getByRole('region', { name: 'Rules' })
  const roundActions = host.getByLabel('Round actions')
  await expect(rulesPanel).toBeVisible()
  await expect(roundActions).toBeVisible()
  await expect(rulesPanel.getByRole('button', { name: 'Resign' })).toHaveCount(0)
  await expect(roundActions.locator('.quick-rules')).toHaveCount(0)
  const rulesBox = await rulesPanel.boundingBox()
  const roundActionsBox = await roundActions.boundingBox()
  expect(rulesBox).not.toBeNull()
  expect(roundActionsBox).not.toBeNull()
  expect(roundActionsBox!.y).toBeGreaterThan(rulesBox!.y + rulesBox!.height)
  ...
  const mobileRulesBox = await rulesPanel.boundingBox()
  const mobileRoundActionsBox = await roundActions.boundingBox()
  expect(mobileRulesBox).not.toBeNull()
  expect(mobileRoundActionsBox).not.toBeNull()
  expect(mobileRoundActionsBox!.y).toBeGreaterThan(mobileRulesBox!.y + mobileRulesBox!.height)
  ...
  await activePage.getByRole('gridcell', { name: 'Row 1, column 1', exact: true }).click()
...
test('mobile lobby and board fit a 390 by 844 viewport', async ({ browser }) => {
  ...
  await expect(page.getByText('Mina wins — five in a row')).toBeVisible({ timeout: 12_000 })
  const replayBoardBox = await page.locator('.match-replay__board').boundingBox()
  const replayWinnerBox = await page.locator('.match-replay__winner').boundingBox()
  expect(replayBoardBox).not.toBeNull()
  expect(replayWinnerBox).not.toBeNull()
  expect(replayWinnerBox!.x).toBeGreaterThanOrEqual(replayBoardBox!.x)
  expect(replayWinnerBox!.x + replayWinnerBox!.width).toBeLessThanOrEqual(
    replayBoardBox!.x + replayBoardBox!.width,
  )
  expect(replayWinnerBox!.y + replayWinnerBox!.height).toBeLessThan(
    replayBoardBox!.y + replayBoardBox!.height / 3,
  )
  ...
  await expect
    .poll(
      async () => (await mobileFirstPoint.isEnabled()) || (await opponentFirstPoint.isEnabled()),
    )
    .toBe(true)
...
test('an upgraded guest can continue from another device', async ({ browser }) => {
  ...
  await registerDialog.locator('form').getByRole('button', { name: 'Create account' }).click()
...
test('a finished round appears in personal leaderboard history', async ({ browser }) => {
  ...
  await expect(host.getByRole('button', { name: 'Sign out' })).toBeVisible({ timeout: 10_000 })
